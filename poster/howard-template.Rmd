---
title: "White Flight and Residential Segregation in Mobile County, AL: Aligning Historical Indicators and U.S. Census Bureau Data to Model Dissimilaritiy"
author: 
  - name: Ethan Gwinn
    affil: 1, 2, 5
  - name: Nathan Alexander, PhD
    affil: 3, 4, 5

affiliation:
  - num: 1
    address: "Department of Sociology, College of Arts and Sciences"
  - num: 2
    address: "Department of Economics, College of Arts and Sciences"
  - num: 3
    address: "Department of Curriculum and Instruction, School of Education"
  - num: 4
    address: "Center for Applied Data Science and Analytics (CADSA), Graduate School"
  - num: 5
    address: "Quantitaive Histories Workshop"

column_numbers: 4

logoright_name: "img/qs-logo.png"
logoleft_name: "img/hu-logo.jpg"

titlebox_borderwidth: "0.25cm"

primary_colour: "#ffffff"
secondary_colour: "#5F6062"
accent_colour: "#E51937"

titlebox_bordercol: "#003A63"
title_textcol: "#003A63"
author_textcol: "#003A63"
affiliation_textcol: "#003A63"

columnline_col: "#E51937"
columnline_style: solid

sectitle_textcol: "#ffffff"
sectitle_bgcol: "#003A63"
sectitle_bordercol: "#003A63"
sectitle2_textcol: "#003A63"

output: 
  posterdown::posterdown_html:
    css: hu-poster.css

bibliography: references.bib
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
 # cache = FALSE,
  fig.width = 3,
  fig.height = 2,  
  fig.env="figure",
  dpi = 300
)

# Ensure required packages are installed and loaded
if (!requireNamespace("posterdown", quietly = TRUE)) {
  pak::pak("brentthorne/posterdown")
}
if (!requireNamespace("pagedown", quietly = TRUE)) {
  pak::pak("rstudio/pagedown")
}

library(tidycensus)
library(dplyr)
library(ggplot2)
library(sf)
library(viridis)
library(scales)
library(readr)

library(posterdown)
library(tidyverse)
library(ggplot2)
library(scales)
library(here)
here::i_am("howard-template.Rmd") # if you change the name of your file, update this
```

```{=html}
<style>
@page {
size: 48in 36in;
margin: 2in;
}
</style>
```

```{r, eval=F, include=FALSE}
# To render both HTML and PDF versions:
rmarkdown::render("howard-template.Rmd")

pagedown::chrome_print(
  "howard-template.html",
  output = "howard-template.pdf",
  options = list(
    paperWidth = 48, 
    paperHeight = 36
    )
)
```

# Abstract

This study analyzes demographic shifts in Mobile County, AL from 1960 to the present to assess the persistence of residential segregation. By utilizing decennial census data and spatial analysis, the research examines how structural and historical forces continue to drive spatial inequality in the post–civil rights era.

# Overview

**White Flight and Segregation**. White flight occurs when white individuals abandon integrated spaces to avoid growing minority populations and stall racial progress. This migration is driven by the desire to protect "whiteness as property," where distancing themselves from other races preserves their social and economic capital (Crowder & South, 2008). In Mobile, desegregation efforts were only made when they protected the elite’s economic interests and established new, spatial forms of white exclusivity (Bell, 1980).

**Segregation Facilitators**.

-   *Whiteness as property*: Whiteness is not just viewed as a racial identity, but as a legal and social asset that operates at a systems level. It posits that European ancestry carries a distinct set of inherent privileges, sociopolitical status, and expectations of entitlement that function as a protected form of property (Donner, 2021).
-   *Interest Convergence*: Racial equality is achieved when the interests of Black people align with the self-interests of a white majority (Bell, 1980).
- *Antiblackness*: We also bridge in contemporary discussions from the field of Black studies regadring conceptions of antiblackness.

# Literature Review

-   Between 1970-1980, Mobile County saw a significant decline in public school enrollment (approx. 11,500 students), directly correlating with a rise in private school attendance and suburban migration (Pride, 2022). This period highlights a distinct pattern of "white flight."
-   Research indicates that white residents often undervalue racially integrated areas due to an "aversion to living near black neighbors" and inflated perceptions of crime (Crowder & South, 2008). These subjective assessments frequently override objective neighborhood quality, driving residential turnover.

# Research Questions

*How have Black and white racial demographics across tracts in Mobile County evolved from 1980-2020?*

# Mathematical Model

$$
D = \frac{1}{2} \sum_{i=1}^{n} \left| \frac{b_i}{B} - \frac{w_i}{W} \right|
$$

where

- $N$ is the number of census tracts,
- $b_i$ is the population of Black residents in unit $i$,
- $B$ is the total population of Black residents in the geographic area,
- $w_i$ is the population of white residents in unit $i$,
- $W$ is the total population of white residents in the geographic area.

## Hypothesis

Levels of residential segregation will remain persistently high, reflecting the continued influence of structural factors such as white flight and path-dependent patterns of residential sorting.

## Limitations

A significant limitation of this analysis was the inability to analyze NHGIS data from 1960, and 1970. Accessing these decades would have provided critical insights into the baseline of segregation before and during the most volatile periods of the Civil Rights Movement and initial court-ordered desegregation.

# Methods and Data

This study uses historical census data from the U.S. Census Bureau to examine changes in Black and white residential distribution across census tracts in Mobile County from 1960 to 2010. Data are analyzed using spatiotemporal demographic mapping, involving longitudinal comparison of tract-level racial composition and the calculation of segregation indices across decennial census years. Our interpretation of observed trends is informed by a Critical Race Theory framework, particularly the concepts of interest convergence and whiteness as property, to contextualize segregation patterns over time.

# Findings

For 1980, $D = 0.771$.

```{r, include=F, message=F, warning=F}
race2_1980 <- readRDS("data/1980_race_map.rds")
glimpse(race2_1980)
```

```{r, message=F, warning=F}
ggplot(race2_1980) +
  geom_sf(aes(fill = pct_black), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    limits = c(0, 1),
    labels = scales::percent_format(accuracy = 1),
    guide = guide_colorbar(
      barwidth = 0.7,
      barheight = 6
    )
  ) +
  labs(
    title = "Mobile County, AL (1980)",
    subtitle = "Index of Dissimilarity = 0.771",
    fill = "% Black"
  ) +
  coord_sf(expand = FALSE) +
  theme_minimal() +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank(),
    legend.position = "right",
    legend.box = "vertical",
    plot.margin = margin(5, 5, 5, 5)
  )
```

We use the spatiotemporal analysis to track changes in $D$ from 1980 to 2020. We find that segregation remained high with some slight, but significant, decreases due to public policies.

![](img/1980-2010-seg.png){width="90%"}

```{r, include=F, warning=F, messages=F}
library(tidycensus)
library(dplyr)
library(sf)
library(ggplot2)
library(viridis)

options(tigris_use_cache = TRUE)

race_2020 <- get_acs(
  geography = "tract",
  state = "AL",
  county = "Mobile",
  year = 2020,
  variables = c(
    total = "B02001_001",
    white = "B02001_002",
    black = "B02001_003"
  ),
  geometry = TRUE
) %>%
  select(GEOID, variable, estimate, geometry) %>%
  tidyr::pivot_wider(names_from = variable, values_from = estimate) %>%
  mutate(
    pct_black = black / total
  )
```

```{r, include=F, warning=F, messages=F}
ggplot(race_2020) +
  geom_sf(aes(fill = pct_black), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    labels = scales::percent_format(accuracy = 1)
  ) +
  labs(
    title = "Black Population Share by Census Tract (Mobile County, 2020)",
    fill = "% Black"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank(),
    panel.grid = element_blank()
  )
```

```{r, include=F, message=F, warning=F}
library(dplyr)

seg_2020 <- race_2020 %>%
  mutate(
    white = white,
    black = black
  )

B <- sum(seg_2020$black, na.rm = TRUE)
W <- sum(seg_2020$white, na.rm = TRUE)

d_2020 <- seg_2020 %>%
  mutate(
    black_share = black / B,
    white_share = white / W,
    diff = abs(black_share - white_share)
  ) %>%
  summarise(D = 0.5 * sum(diff, na.rm = TRUE))

d_2020

d_value <- d_2020$D
```

We separate data for 2020 due to Covid-19.

```{r, message=F, warning=F}
ggplot(race_2020) +
  geom_sf(aes(fill = pct_black), color = NA) +
  scale_fill_viridis_c(
    option = "magma",
    limits = c(0, 1),
    labels = scales::percent_format(accuracy = 1),
    guide = guide_colorbar(
      barwidth = 0.7,
      barheight = 6
    )
  ) +
  labs(
    title = "Mobile County, AL (2020)",
    subtitle = paste0("Index of Dissimilarity = ", round(d_value, 3)),
    fill = "% Black"
  ) +
  coord_sf(expand = FALSE) +
  theme_minimal() +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank(),
    legend.position = "right",
    legend.box = "vertical",
    plot.margin = margin(5, 5, 5, 5)
  )

```

```{r, eval=F, message=F, warning=F}
# removing old 2020 map so that it matches the 1980s map
ggplot(race_2020) +
  geom_sf(aes(fill = pct_black), color = NA) +
  scale_fill_viridis_c(
  option = "magma",
  labels = scales::percent_format(accuracy = 1),
  guide = guide_colorbar(
    barwidth = 0.7,
    barheight = 6
  )
) +
  labs(
    title = "Mobile County, AL (2020)",
    subtitle = paste0("Index of Dissimilarity = ", round(d_value, 3)),
    fill = "% Black"
  ) +
  theme_minimal() +
  theme(
    axis.text = element_blank(),
    axis.title = element_blank()
  )
```

# Discussion

The 11.3% decrease in the Dissimilarity Index between 1980 and 1990 possibly represents a period of transitory diversity,  rather than sustainable integration. As white families abandoned the urban core, census tracts briefly appeared integrated before reverting to new patterns of segregation. 

Our data reveals a disconnect between legal milestones and spatial reality. While MCPSS achieved "unitary" status in 1997 , the 2020 Index of Dissimilarity remains high at 0.552. This suggests that while legal oversight ended, private-sector white flight and path-dependent sorting persisted as mechanisms to maintain racial distance and preserve whiteness as property.

The 1988 Mobile Plan exemplifies Interest Convergence. By establishing elite magnet schools like Council Traditional and Phillips Preparatory, the county incentivized white families—who had withdrawn 11,500 students the prior decade—to remain in the system. This protected “whiteness as property" by offering exclusive educational assets in exchange for technical legal compliance, effectively stalling deeper residential integration. 

The 2020 estimate reflects a "frozen" snapshot. The pandemic stalled residential mobility and disrupted Census operations in high-density tracts. Consequently, the 0.552 index may mask the true contemporary movement of residential sorting, as lockdowns temporarily suppressed the migration patterns our model tracks. 

# Conclusion

Our analysis reveals that residential segregation in Mobile County is a persistent structural force. To deepen this understanding, future research should extend the longitudinal dataset to 1950 and introduce qualitative interviews to capture the lived experiences and oral histories that quantitative data alone cannot reveal. 

# Acknowledgements

This work was supported by funding from the Alfred P. Sloan Foundation (Grant 2023-21062).

# References

![Scan to see our timeline and references!](img/qr-code.png){width=50%}

